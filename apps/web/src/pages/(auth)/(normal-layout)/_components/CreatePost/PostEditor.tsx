...
import { ChevronDown, Smile, GripHorizontal, XIcon, Globe } from 'lucide-react'
...
interface PostEditorProps {
  onSelectPublicType: () => void
  onClose: () => void
  publishType: string
}

export default function PostEditor({ onSelectPublicType, publishType, onClose }: PostEditorProps) {
  ...
  const handleCreatePost = async () => {
    try {
      ...
      await createPost({ content: value, publishType, media: urls })
      onClose()
    ...
    <>
      ...
      <div className="grid grid-flow-row gap-1 p-2">
        <div className="flex gap-2 p-4">
          <img className="size-12 rounded-full" src={me?.avatar} alt="" />
          <div className="space-y-1">
            <p className="text-sm font-bold">
              {me?.firstName} {me?.lastName}
            </p>
            <div
              onClick={onSelectPublicType}
              className="flex cursor-pointer items-center gap-1 rounded-sm bg-gray-200 px-2 py-0.5  text-sm font-medium"
            >
