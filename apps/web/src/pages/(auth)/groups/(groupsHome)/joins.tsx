...
export default function Joins() {
  const groups: { data: Group[] } = useOutletContext()
  const totalGroup = groups?.data.length
  return (
    <div className="">
      <p className="">Tất cả các nhóm mà bạn đã tham gia ({totalGroup})</p>
      <div className="flex flex-wrap gap-3">
        {groups?.data.map(group => (
          <div key={group.name} className="flex flex-col gap-5 rounded-lg bg-white p-4">
